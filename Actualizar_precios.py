...
def generar_excel_automatizado():
    ...
    # Configuramos para que siempre tome los últimos 8 días desde el momento actual
    end_date = datetime.now(pytz.utc)
    start_date = datetime(2026, 1, 1, 0, 0, 0, tzinfo=pytz.utc) # O el inicio que prefieras
    ...
    def styler_completo(x):
        ...
        for g in x['grupo_hora'].unique():
            ...
            hora_num = int(g[-2:])
            bg_row = 'background-color: #F9F9F9;' if hora_num % 2 == 0 else 'background-color: #EBEBEB;'
